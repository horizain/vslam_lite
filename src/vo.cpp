...
void VisualOdometry::handleLoopCorrection(const SE3& T_loop_curr,
    ...
    Workset ws;
    {
        ...
        for (const auto& kf : all_kfs) {
            ...
            auto snap = snapshotFrame(kf, snap_cache, nullptr, false);
    ...
    // ---- 阶段 2：锁外计算 ----
    // 2a. 快照 KF + 快照点塞入临时 Map。全局 BA 会通过 getMapPoint
    //     查找点，必须注册快照点，否则快照模式下不会精修任何地图点。
    auto tmp_map = std::make_shared<Map>();
    for (auto& s : ws.snap_kfs) {
        tmp_map->insertKeyFrame(s);
        for (const auto& smp : s->map_points)
            if (smp) tmp_map->insertMapPoint(smp);
    }
    {
    ...
    // 2b'. 将粗同步后的点坐标作为全局 BA 初值写入快照点。
    {
        std::unordered_map<unsigned long, Vec3> synced;
        synced.reserve(ws.points.size());
        for (const auto& [mp, pos] : ws.points) synced.emplace(mp->id, pos);
        for (auto& s : ws.snap_kfs) {
            for (auto& smp : s->map_points) {
                if (!smp) continue;
                auto it = synced.find(smp->id);
                if (it != synced.end()) smp->pos_w = it->second;
            }
        }
    }
    ...
    // 明确保持有序，避免未来 Map 容器实现变化破坏 lower_bound 前置条件。
    std::ranges::sort(corrections, {}, &std::pair<unsigned long, SE3>::first);
    ...
    // 异步优化期间前端可能继续追加有效位姿。先验证快照仍是实时轨迹前缀，
    // 再把新增尾部变换到新世界系；验证失败时必须在地图写回前整笔放弃。
    {
        std::lock_guard<std::mutex> lock(traj_mutex_);
        bool snapshot_is_prefix = pose_trajectory_.size() >= ws.traj.size()
            && traj_frame_ids_.size() >= ws.traj_ids.size();
        if (snapshot_is_prefix) {
            for (size_t i = 0; i < ws.traj_ids.size(); i++) {
                if (traj_frame_ids_[i] != ws.traj_ids[i]) {
                    snapshot_is_prefix = false;
                    break;
                }
            }
        }
        if (snapshot_is_prefix && !corrections.empty()) {
            const SE3 tail_correction = corrections.back().second;
            for (size_t i = ws.traj.size();
                 i < pose_trajectory_.size() && i < traj_frame_ids_.size(); i++) {
                const SE3 Twc_new = tail_correction * pose_trajectory_[i].inverse();
                new_traj.push_back(Twc_new.inverse());
                ws.traj_ids.push_back(traj_frame_ids_[i]);
            }
        } else if (!snapshot_is_prefix) {
            LOG_WARN("Loop trajectory writeback skipped: live trajectory no longer extends snapshot");
            return;
        }
    }
    ...
    {
        std::lock_guard<std::mutex> lock(traj_mutex_);
        // 地图写回期间前端仍可能新增极少量轨迹，再做一次尾部追赶，保证最终
        // 替换发生在同一把锁内且不会覆盖刚追加的位姿。
        if (!corrections.empty()) {
            const SE3 tail_correction = corrections.back().second;
            for (size_t i = ws.traj_ids.size();
                 i < pose_trajectory_.size() && i < traj_frame_ids_.size(); i++) {
                const SE3 Twc_new = tail_correction * pose_trajectory_[i].inverse();
                new_traj.push_back(Twc_new.inverse());
                ws.traj_ids.push_back(traj_frame_ids_[i]);
        ...
        traj_frame_ids_ = std::move(ws.traj_ids);
    }
...
void VisualOdometry::runBackendLocalBA(const std::vector<Frame::Ptr>& window) {
    ...
    std::vector<Frame::Ptr> snaps, reals;
    {
        ...
        for (const auto& kf : window) {
            ...
            reals.push_back(kf);
            snaps.push_back(snapshotFrame(kf, snap_cache, nullptr, false));  // BA 不消费描述子
        }
    }
