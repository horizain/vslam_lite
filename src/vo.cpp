...
=======
    Workset ws;
    {
        ...
        for (const auto& kf : all_kfs) {
            ...
            // snap_cache 复用同 id 快照点对象（防止 BA 精修值被同 id 快照点覆盖）
    ...
    // 快照 KF + 快照点入临时 Map（optimizer 用 getMapPoint 查点；快照点
    // 必须注册，否则全局 BA 查不到任何点 → 点不精修 → 回环后点/位姿不一致）
...
=======
    // 2b'. 把点同步结果写进快照点（全局 BA 的初值）——保证全局 BA 从
    //      同步后值收敛，写回时 BA 精修值覆盖同步值
    {
...
>>>>>>> Stashed changes
    ...
    //    corrections 必须按 KF id 排序：lower_bound 要求有序，而
    //    getAllKeyFrames() 的 unordered_map 迭代序无序——不排序是 UB，
    //    插值会错乱 → 重写后的轨迹出现 10m+ 毛刺（实测，2026-08-06）。
...
=======
    ...
    // ---- 阶段 3：锁内写回（独占）----
    {
        ...
        // 写回顺序（关键，2026-08-06 修复）：先写全量点同步值（粗值），再写
        // 快照点（全局 BA 精修值）——顺序颠倒会让粗值覆盖精修值，回环校正后
        // 点全部退化为无重投影精修状态 → 后续 PnP 用粗点 → 轨迹毛刺 + 漂移
        // 全量点同步结果 → 真点（快照指针，shared_ptr 保证存活）
        for (const auto& [mp, pos] : ws.points) {
            auto real = map_->getMapPoint(mp->id);
            if (real) real->pos_w = pos;
        }
        // 全局 BA 优化的快照点 → 真点（snap_cache 保证同 id 单对象，即精修值）
...
Frame::Ptr VisualOdometry::snapshotFrame(
    ...
    for (size_t i = 0; i < kf->map_points.size(); i++) {
        ...
        // 关键：同一真点被多个快照 KF 引用时必须复用同一快照点对象
        //（按 id 缓存）——否则 BA 优化的是 X1、写回遍历先遇 X1（BA 值）
        // 后遇 X2（同步值）覆盖 → 部分点未精修 → 点/位姿不一致 → 轨迹跳变
...
void VisualOdometry::runBackendLocalBA(const std::vector<Frame::Ptr>& window) {
    ...
    std::vector<Frame::Ptr> snaps, reals;
    {
        ...
        std::unordered_map<unsigned long, MapPoint::Ptr> snap_cache;  // 同 id 快照点复用
...
void VisualOdometry::runBackendLoopClosure(const Frame::Ptr& curr_kf) {
    ...
    // 锁内快照当前 KF（detectLoop 只需描述子/位姿；无需点）
    Frame::Ptr snap_curr;
    {
        std::shared_lock<std::shared_mutex> lock(map_mutex_);  // P1：快照为读路径
        if (!map_->getKeyFrame(curr_kf->id)) return;
        std::unordered_map<unsigned long, MapPoint::Ptr> empty_cache;
        snap_curr = snapshotFrame(curr_kf, empty_cache);
    }
