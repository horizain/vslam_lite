...
> 状态：M0 完成；M1 全部完成（均确定性验收通过）；M2 完成（输入队列 +
> 资源预算 + 结构化指标/soak，§6.2 参数已按 §6.5 RSS 硬门槛实测标定，
> §3.38；RSS <1GiB 对 KITTI 00 整序列未达标——根因链 LOST→多子地图累积
> +词袋 350MB，修复依赖 M3.1/M4/M5，2h 档 nightly 观测）；
> 下一步 M3.1 视觉前端质量门
...
| M2 实时/资源/指标 | 完成 | M2.1 输入队列（bounded_queue 固定容量 + 丢最旧保最新 + Localizer 异步 worker + hwm/dropped，11 项测试）；M2.2 资源预算（§6.3 六步固定顺序回收 + Map::removeKeyFrame/最近命中旁路统计 + robot.yaml MapBudget 段，14 项测试，KITTI 00 1000 帧确定性逐位一致）；M2.3 结构化指标（`metrics.{h,cpp}` §6.4 全字段 JSON/CSV + BackendScheduler 调度统计 + Localizer 采集接线 + `soak_test.py`，11 项指标测试 + CTest test_soak）。M2 遗留清理（§3.38）：§6.2 参数按 §6.5 RSS<1GiB 硬门槛实测标定（KF700/点6万/总量500MB + overhead 实测化）+ 预算 stopped 恢复振荡修复 + 第 5 步冻结子地图弱陈点回收 + frame latency 只计跟踪帧 + --loop 时间戳偏移 + soak 轮式重构；15/15 CTest、ASan 干净、L2 快速档 PASS（p99 worst 68.7ms）。**未达标项**：KITTI 00 整序列 RSS 峰值 1523MB >1GiB（LOST→多子地图累积 + 词袋 350MB，M3.1/M4/M5 修复）；下一步 M3.1 质量门 |
