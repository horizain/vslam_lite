...
> 状态：M0 完成；M1.1 PoseGate、M1.2 Relocalizer、M1.3 BackendScheduler 完成
> （均确定性验收通过），M1.4 FrontendTracker 进行中
...
| M1 模块拆分 | 进行中 | M1.1 PoseGate + M1.2 Relocalizer + M1.3 BackendScheduler 已落地（11+9+7 项测试，8/8 CTest 全过，1000 帧确定性验收通过：轨迹逐位一致、旋转差 5.5e-17 rad）；M1.6 基准门已落地（`--metrics-json` + benchmark.py v2 多轮 mean/std/worst + `.githooks/pre-commit`）；下一步 M1.4 FrontendTracker |
