...
> 状态（2026-08-11 长程复测）：M0/M1 的结构已落地；M2 的“完成”结论仍为
> **实现完成、产品验收未通过**。新鲜构建审计发现旧门禁可在数据路径错误、0 帧
> soak、空轨迹和错误聚合方向下假绿；KITTI 00 还复现了 KF 硬预算触顶导致周期
...
> 2026-08-18：新增 default=6 核/12GiB、mobile=4 核/6GiB 运行资源契约、Linux affinity
> 与后台 worker 固定、双目灰度/LK 和关键帧匹配复用、BA/回环任务年龄与验证预算；提交门
> 强制 RSS/轨迹非空/有限值检查。完整长程 p99、LOST、mobile 质量和 TBB OS worker 数量
> 仍需独立产品验收，不能把快速门结果外推为生产就绪。
...
### 0.2 可执行验收协议（给小模型执行任务时强制使用）

详细文档不是测试 oracle。每个任务除文字规格外，必须同时附带以下机器可判定证据：

1. **先红后绿**：先在父版本运行新增测试并保存失败输出，再实现；不能只展示修改后通过。
2. **非空工作证明**：数据测试必须断言 `frames_processed > 0`、轨迹/状态非空、GT
   匹配数达到门槛。0 帧、0 个测试、空文件不得视为成功。
3. **负例控制**：错误数据路径、空目录、损坏配置和不可读输入必须非零退出；负例若通过，
   正例结果无效。
4. **新鲜构建**：提交门在临时目录重新 configure/build，并使用
   `ctest --no-tests=error`；禁止复用开发者已有 `build/`。
5. **分层门禁**：L0 单元/公式，L1 小型确定性轨迹，L2 真实数据 + GT，L3 长程
   资源/复杂度。低层通过不能替代高层。
6. **黄金数据隔离**：行为实现任务不得同时更新 reference/golden；更新黄金数据需要独立
   评审和 A/B 指标批准。
7. **数学与线程不变量**：涉及位姿时写明 `T_ab` 方向并用非单位旋转/平移测试；涉及异步
   时列出对象所有者，worker 不得写前端帧内快照。
8. **规模反例**：批处理/回收必须用接近生产数量级的复杂度测试；几十个对象上的单测不能
   证明 4 万点路径可用。
9. **独立评审**：评审模型直接读 diff、重跑命令和复核原始 JSON，不复述执行模型总结。
10. **停止条件**：ATE、LOST、跳变、RSS 或 p99 任一退化超过任务阈值即拒绝补丁，不能
    通过放宽门限、关功能或改 reference 消除红灯。
