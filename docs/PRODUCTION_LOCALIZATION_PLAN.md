...
> 状态：M0 完成；M1.1 PoseGate 完成（确定性验收通过），M1.2 Relocalizer 进行中
...
| M1 模块拆分 | 进行中 | M1.1 PoseGate 已落地（11 项测试 + 1000 帧确定性验收）；M1.6 基准门已落地（`--metrics-json` + benchmark.py v2 多轮 mean/std/worst + `.githooks/pre-commit`）；下一步 M1.2 Relocalizer |
