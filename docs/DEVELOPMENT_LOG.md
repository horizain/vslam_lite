...
> 创建日期: 2026-07-30
> 最后更新: 2026-08-04 (追认修复 verifyLoop 位姿图测量 T_loop_curr 多一个逆的问题)
...
- **追认修复（2026-08-04）**：`matToSE3(rvec, tvec)` 实为 T_wc 矩阵，旧代码又 `.inverse()`
  导致 `T_loop_curr = X_loop⁻¹·X_curr⁻¹`，代入位姿图约束 `X_curr = X_loop·T_rel` 会得到
  恒等式矛盾——这是"回环校正破坏轨迹"的残留根因，现已改为 `X_loop⁻¹·X_curr`；
