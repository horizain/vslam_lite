...
**右上默认 = 世界系 2D X-Z 轨迹图**。面板中的 `Show 3D map` 默认关闭，打开后
才切换为可交互 3D 地图视图：鼠标左键旋转、右键平移、滚轮缩放（`r` 复位），
并叠加世界系地图点云（绿色）、3D 轨迹折线（青色）、当前相机视锥与 XZ 参考网格。
`Follow camera` 开启时 3D 视角自动跟随当前相机，用户拖动后约 0.8 s 内不会被重设。
面板可切换 `Show 3D map` / `Show map points` / `Show camera` / `Show grid` / `Show trail`。
...
当前项目已接入可选 DBoW3 回环检测、Essential Anchor Graph、Atlas Submap Graph
和异步后端；关闭回环时仍可作为纯 VO 运行。同子图回环只在短临界区里冻结
前缀/提交，g2o 在 Map 锁外运行；求解期间前端新增的尾段会在提交时重基。全局 BA
可配置但默认关闭，跨子地图地图点融合尚未实现：

1. **回环检测**：用词袋（DBoW3）判断"我是不是回到了老地方"→ 检测到回环说明漂移可纠正
2. **位姿图优化**：子图内只优化有硬上限的关键锚点，跨子图只优化 `T_ws`，不再把全量 KF 都送进 g2o
3. **全局 BA**：回环后把全部关键帧+地图点重新优化一遍

`config/mobile.yaml` 的 `LoopClosure.retrieval_backend: flat_dbow3` 不是降低精度的
哈希替代品：它读取同一 `.dbow3` 词表，保留相同层次量化和 TF-IDF/L1 打分，只把百万级
词表节点从重对象树改成连续 SoA，并把关键帧索引硬限制为 256。为了不让 FIFO 在长序列
只保留末尾，它固定首帧并对余下历史做确定性 reservoir-style 采样。检索后先积累连续地点假设，
mobile 通过 `mature_verification_limit: 4` 每轮最多对 4 个成熟 BoW 地点和 1 个位置先验
做 PnP；默认 0 保持高精度档全候选验证，几何接受门也不因 mobile 降低。

不要把 `compact_binary` 当作 mobile 默认项：它在 KITTI 00 实验中虽把峰值 RSS 降到
`221376KiB`（约 216MiB），却使 5 个预期回环全部漏检。当前 `flat_dbow3` 的两轮完整
序列峰值 RSS 为 `290100～290244KiB`（约 283MiB），有效帧为 `4484/4541` 和
`4465/4541`，分别有 3、4 次 LOST；两轮均为 1/5 回环召回、SE3 ATE `30.434～53.900m`、
p99 `211.3～347.0ms`。这说明内存布局和验证级联有效，却还没有解决前端长程跟踪波动，
也没有达到 ARM 产品门。

定位输出分两层：`T_ob` 是给控制器的连续 odom 位姿，`T_wo` 是回环可更新的全局校正，
规划/地图位姿为 `T_wb = T_wo * T_ob`。回环不回写 `T_ob` 历史。当前这是纯视觉相邻位姿
积分；IMU/ESKF 高频传播仍属后续 M6，不得把已有分层误写成完整惯导融合。
