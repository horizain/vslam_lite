...
| `LOST` | PnP 失败且匹配不足 | 依次与关键帧匹配做 PnP（先最近 5 帧，再全量），内点≥20 恢复 |
...
### 4.4 滑动窗口 Local BA（insertKeyFrame → selectLocalWindow）
...
Pangolin 双窗口：**左 = 2D 轨迹图**（x-z 俯视），**右 = 彩色视频流 + 绿色特征点**。独立渲染线程 + `std::mutex` 保护共享数据——这是多线程编程的最小示范。
