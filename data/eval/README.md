...
| `abcd21_traj.txt` | commit 失败保护后：**仍 85.4m 跳变（当前未解决问题复现）** |
...
```bash
# 当前 HEAD 状态（§3.21 提交）复现 85m 跳变：
./build/bin/run_slam datasets/sequences/00 config/default.yaml /tmp/repro.txt --headless
